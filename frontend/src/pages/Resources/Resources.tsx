...
export default function Resources() {
  ...
  const { pageReportId } = useParams<{ pageReportId: string }>();
  const [data, setData] = useState<any>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const loadData = async () => {
      if (!selectedProjectId || !pageReportId) return;
      try {
        const response = await apiClient.get(
          `/resources/${selectedProjectId}/${pageReportId}`
        );
    ...
    loadData();
  }, [selectedProjectId, pageReportId]);
          ...
          {data?.issues && data.issues.length > 0 ? (
            <ul className="space-y-2">
              {data.issues.map((issue: any) => (
                <li
                  key={issue.id}
                  className="p-4 bg-red-50 dark:bg-red-900/20 rounded"
                >
