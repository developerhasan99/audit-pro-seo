...
export const pageReports = pgTable(
  "pagereports",
  {
    ...
    blockedByRobotstxt: boolean("blocked_by_robotstxt")
      .default(false)
      .notNull(),
  ...
  (table) => ({
    ...
    crawlIdUrlHashUnique: uniqueIndex("pagereports_crawl_id_url_hash_idx").on(
      table.crawlId,
      table.urlHash
    ),
  })
...
export const hreflangs = pgTable("hreflangs", {
  id: serial("id").primaryKey(),
  pagereportId: integer("pagereport_id")
    .notNull()
    .references(() => pageReports.id, { onDelete: "cascade" }),
  crawlId: integer("crawl_id")
    .notNull()
    .references(() => crawls.id, { onDelete: "cascade" }),
...
export const links = pgTable("links", {
  ...
  crawlId: integer("crawl_id")
    ...
    .references(() => crawls.id, { onDelete: "cascade" }),
  url: varchar("url", { length: 2048 }).notNull(),
  scheme: varchar("scheme", { length: 20 }).notNull(),
...
export const issuesRelations = relations(issues, ({ one }) => ({
  pageReport: one(pageReports, {
    fields: [issues.pagereportId],
    references: [pageReports.id],
  }),
